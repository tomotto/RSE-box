function y = maxNormCDF(x, mu, sigma, rho)
%MAXNORMCDF   Maximum of two correlated Gaussian random variables.
% Y = MAXNORMCDF(X,MU,SIGMA,RHO) returns the cdf of the maximum of two
...
%
% See also maxNormPDF, minNormPDF, minNormCDF
...
y = 1 - ( 0.5 * normcdf( x1 ) - t1  + ...
          0.5 * normcdf( x2 ) - t2  );

% Shift the function for x-values between mu1 and mu2
idx = ( x>mu(1) & x<mu(2)) | (x>mu(2) & x<mu(1) );
y(idx) = y(idx) - 0.5;

% Interpolation for x-values equal to mu1 or mu2
idx = x==mu(1);
if sum(idx)>0
    y(idx) = mean( maxNormCDF( mu(1)+[-1e-10 1e-10], mu, sigma, rho ) );
end
idx = x==mu(2);
if sum(idx)>0
    y(idx) = mean( maxNormCDF( mu(2)+[-1e-10 1e-10], mu, sigma, rho ) );
end
