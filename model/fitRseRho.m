function [fit, logL] = fitRseRho( rt1, rt2, rtR, nStart, nGrid )
%FITRSERHO   Maximum likelihood estimation of the overall RSE model (5 parameters).
% [FIT, LOGL] = FITRSERHO( RT1, RT2, RTR, NSTART, NGRID ) performs a joint
...
% (rho; see Otto & Mamassian, 2012). The second race model parameter eta is
% assumed to be zero (context invariance). Input arguments RT1, RT2, and
...
% parameters in the order mu1, mu2, sigma1, sigma2, and rho. LOGL contains
...
% Select start values for RHO using a grid search
startR = getStartVal( data, idx, startL, nStart, nGrid );

% Collate lower and upper bounds for the fitting procedure
lowBound = [ 0.75 * startL, -0.99999 ];
uppBound = [ 1.25 * startL,  0.99999 ];

% Function handel for the fitting procedure
custpdf = @( data, mu1, mu2, sigma1, sigma2, rho ) ...
   modelpdf( data, mu1, mu2, sigma1, sigma2, rho, idx );
...
%  Model PDF function
% -------------------------------------------------------------------------
function y = modelpdf( data, mu1, mu2, sigma1, sigma2, rho, idx )
...
yR = maxNormPDF( data(idx==3), [mu1 mu2], [sigma1 sigma2], rho );
...
% Range of start values for RHO
rhoVal = linspace( -0.95, 0.95, nGrid );

% Grid search
logL = zeros( size(rhoVal) );
for ii=1:nGrid
    logL(ii) = sum( log( modelpdf( data, startL(1), startL(2), startL(3), startL(4), rhoVal(ii), idx ) ) );
end
[~, idx] = sort( logL(:), 'descend' );

% Select start values based on grid search
startR = rhoVal( idx(1:nStart) );
